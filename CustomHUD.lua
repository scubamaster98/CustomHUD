...
if RequiredScript == "lib/managers/hud/hudteammate" then
	...
	function HUDTeammateCustom:set_callsign(id)
		if self._is_player then
			...
			self:set_rank(managers.experience:current_level())
		end

		self:call_listeners("callsign", id)
	end

	function HUDTeammateCustom:set_rank(level)
		self:call_listeners("rank", level)
	end
	...
	function PlayerInfoComponent.PlayerInfo:set_rank(level)
		if level then
			local str = string.format("[%s]", tostring(level))
	...
	function PlayerInfoComponent.KillCounter:init(panel, owner, height, settings)
		...
		self._icon = self._panel:bitmap({
			name = "icon",
			texture = "guis/textures/pd2/crimenet_skull", --replaced with something else, og was guis/textures/pd2/cn_miniskull
			color = Color.white,
			h = height,
			w = height,
		})
